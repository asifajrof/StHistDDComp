        ...
        for model_subpath_1 in model_subpaths:
            ...
            for model_subpath_2 in model_subpaths:
                ...
                comparison = ModelComparisonData(
                    model_ref=dataset_model_1,
                    model_cmp=dataset_model_2,
                    metric=Metrics.AMI
                )
                ami_score = comparison.calculate_metric()
                df_confusion_matrix.loc[f"{model_subpath_1.name}_{model_subpath_1.sub_path}", f"{model_subpath_2.name}_{model_subpath_2.sub_path}"] = ami_score
            ...
            comparison = ModelComparisonData(
                model_ref=dataset_model_1,
                model_cmp=dataset_manual_annotations,
                metric=Metrics.AMI
            )
            ami_score = comparison.calculate_metric()
            df_confusion_matrix.loc[f"{model_subpath_1.name}_{model_subpath_1.sub_path}", "Ground_Truth"] = ami_score

            comparison = ModelComparisonData(
                model_ref=dataset_manual_annotations,
                model_cmp=dataset_model_1,
                metric=Metrics.AMI
            )
        
            ami_score = comparison.calculate_metric()
            df_confusion_matrix.loc["Ground_Truth", f"{model_subpath_1.name}_{model_subpath_1.sub_path}"] = ami_score

        df_confusion_matrix["Ground_Truth"]["Ground_Truth"] = 1.0

        output_dir = f"results/ami_confusion_matrices/{sample}/"
        os.makedirs(output_dir, exist_ok=True)
        df_confusion_matrix.to_csv(f"{output_dir}/ami_confusion_matrix.csv")             
